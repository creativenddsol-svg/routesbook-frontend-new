...
const NoticesSection = () => {
  ...
  // Keep dots in sync with scroll (mobile)
  const updateActiveIndex = useCallback(() => {
    const el = railRef.current;
    if (!el || !el.firstChild) return;
    const cardFull = el.firstChild.offsetWidth + 16; // width + gap
    const newIndex = Math.round(el.scrollLeft / cardFull);
  ...
  const scrollToCard = useCallback(
    (index) => {
      const el = railRef.current;
      if (!el || !el.firstChild) return;
      const width = el.firstChild.offsetWidth;
      const gap = 16;
      el.scrollTo({
    ...
    return (
      <section className="w-full py-8 sm:py-12">
        ...
        <div className="lg:hidden w-full overflow-hidden">
          <div className="flex gap-4 overflow-x-auto pl-4 hide-scrollbar">
            {[...Array(4)].map((_, i) => (
              <Skeleton key={i} />
            ))}
          </div>
  ...
  return (
    <section className="w-full py-8 sm:py-12">
      ...
      {/* ---------- Mobile / Tablet rail (fixed left overflow) ---------- */}
      <div className="lg:hidden w-full overflow-hidden">
        <div
          ref={railRef}
