    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center text-gray-700">
        <Icon
          ...
          className="w-5 h-5 mr-3 text-gray-400"
        />
        <span>Operator: {getOperatorText(bus, operatorMap)}</span>
      </div>
...
const BusTableRow = ({ bus, onDelete, operatorMap }) => (
...
const AdminBusList = () => {
  ...
  const operatorMap = useMemo(() => {
    ...
    for (const op of operators || []) {
      o[op._id] = op.fullName || op.name || op.email || "";
    }
    return o;
  }, [operators]);
