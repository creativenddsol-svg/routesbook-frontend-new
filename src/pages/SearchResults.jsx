...
const getMobileDateParts = (dateString) => {
  ...
  return {
    top: dt.toLocaleDateString("en-GB", { day: "2-digit", month: "short" }),
    bottom: dt.toLocaleDateString("en-GB", { weekday: "short" }),
  };
};
...
const BookingDeadlineTimer = ({
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      const t = deadlineTimestamp - Date.now();
      if (t <= 0) {
        setTimeLeft(0);
        clearInterval(id);
        onDeadline?.();
  ...
  return (
    <div className="text-xs font-semibold mt-1 tabular-nums" style={{ color: PALETTE.orange }}>
...
const SearchResults = ({ showNavbar, headerHeight, isNavbarAnimating }) => {
  ...
  /* --- iPhone full-display / notch tint --- */
  useEffect(() => {
    try {
      ...
      // Light status bar area tint like redBus
      ...
      } else theme.content = PALETTE.white;
  ...
  const dateInputRef = useRef(null); // desktop
  const mobileDateInputRef = useRef(null); // mobile
  ...
  const tomorrowStr = toLocalYYYYMMDD(tmr);
  ...
  useEffect(() => {
    const fetchSpecialNotices = async () => {
      try {
        ...
        console.error("Failed to fetch special notices:", e);
      } finally {
  ...
  useEffect(() => {
    const fetchAllBuses = async () => {
      try {
        ...
        console.error("Failed to fetch bus locations for dropdowns", e);
      }
    };
    fetchAllBuses();
  }, []);
      ...
      await Promise.all(
        res.data.map(async (bus) => {
          try {
            ...
            const r = await apiClient.get(`/bookings/availability/${bus._id}`, {
              params: { date: searchDateParam, departureTime: bus.departureTime },
            });
            seatData[key] = {
              available: r.data.availableSeats,
              window: r.data.availableWindowSeats || null,
              bookedSeats: Array.isArray(r.data.bookedSeats)
                ? r.data.bookedSeats.map(String)
                : [],
              seatGenderMap: r.data.seatGenderMap || {},
            };
          } catch (e) {
            const key = `${bus._id}-${bus.departureTime}`;
            seatData[key] = {
  ...
  const { filteredBuses } = useMemo(() => {
    ...
    const searchingToday = searchDateParam === toLocalYYYYMMDD(today);
    return {
      filteredBuses: buses.filter((bus) => {
        ...
        const dep = new Date(Y, M - 1, D, h, m).getTime();

        if (searchingToday && dep <= now.getTime()) return false;
        const deadline = dep - 60 * 60 * 1000;
        if (deadline <= now.getTime()) return false;

        const activeSlots = Object.keys(filters.timeSlots).filter(
          (s) => filters.timeSlots[s]
        );
        const matchTime =
          activeSlots.length === 0 ||
          activeSlots.some((s) => {
            const [start, end] = TIME_SLOTS[s];
            return s === "Night"
              ? h >= start || h < TIME_SLOTS["Morning"][0]
              : h >= start && h < end;
          });
  ...
  const sortedBuses = useMemo(() => {
    return [...filteredBuses].sort((a, b) => {
      const pa = getDisplayPrice(a, from, to);
      const pb = getDisplayPrice(b, from, to);
      switch (sortBy) {
        case "price-asc":
          return pa - pb;
        case "price-desc":
  ...
  const readableDateForHeader = useMemo(() => {
    ...
    const [y, m, d] = searchDateParam.split("-");
  ...
  const NoResultsMessage = () => {
    ...
    return (
      ...
      >
        ...
        />
        <h3 className="text-2xl font-bold mb-2" style={{ color: PALETTE.textDark }}>
          {hasActiveFilters ? "No Buses Match Your Filters" : "No Buses Available"}
        </h3>
        <p className="max-w-md mx-auto mb-6" style={{ color: PALETTE.textLight }}>
          {hasActiveFilters
      ...
      <div className="p-6 space-y-8 lg:p-0">
        ...
        >
          <h3 className="text-xl font-bold flex items-center gap-3" style={{ color: PALETTE.textDark }}>
            <FaSlidersH className="lg:hidden" style={{ color: PALETTE.accentBlue }} /> {headerText}
          </h3>
          {isMobile ? (
            ...
            {Object.keys(TIME_SLOTS).map((slot) => (
              <button
                ...
                className={`px-2 py-2 text-sm font-semibold rounded-lg border-2 transition-all ${
                  filters.timeSlots[slot] ? "text-white border-blue-500" : "border-gray-200"
                }`}
                style={{
                  backgroundColor: filters.timeSlots[slot] ? PALETTE.accentBlue : PALETTE.white,
                  color: filters.timeSlots[slot] ? PALETTE.white : PALETTE.textDark,
                }}
              >
                {slot}
              </button>
        ...
        <section>
          ...
          <div className="text-sm mt-2 text-center font-medium" style={{ color: PALETTE.textLight }}>
            Up to <span className="font-bold" style={{ color: PALETTE.primaryRed }}>
              Rs. {filters.maxPrice}
            </span>
          </div>
        </section>
  ...
  const handleSeatToggle = (bus, seat) => {
    ...
    setBusSpecificBookingData((prev) => {
      ...
      const isSelected = current.selectedSeats.includes(String(seat));
      const canAdd = !isSelected && current.selectedSeats.length < 4;

      const nextSelected = isSelected
        ? current.selectedSeats.filter((s) => s !== String(seat))
        : canAdd
        ? [...current.selectedSeats, String(seat)]
        : (toast.error("🚫 You can select a maximum of 4 seats."), current.selectedSeats);

      const nextGenders = { ...(current.seatGenders || {}) };
      if (isSelected) delete nextGenders[String(seat)];
      else if (canAdd && !nextGenders[String(seat)]) nextGenders[String(seat)] = "M";

      return {
        ...prev,
        [key]: { ...current, selectedSeats: nextSelected, seatGenders: nextGenders },
      };
    });
  };
  ...
  useEffect(() => {
    ...
    const [currentId, currentTime] = expandedBusId.split("-");
    const currentBus = buses.find((b) => b._id === currentId && b.departureTime === currentTime);
    ...
    if (currentBus.convenienceFee) {
      if (currentBus.convenienceFee.amountType === "percentage")
        cf = (basePrice * currentBus.convenienceFee.value) / 100;
      else cf = currentBus.convenienceFee.value * selectedSeats.length;
    }
    const total = basePrice + cf;

    if (total !== busData.totalPrice || basePrice !== busData.basePrice) {
      setBusSpecificBookingData((prev) => ({
        ...prev,
        [expandedBusId]: { ...prev[expandedBusId], basePrice, convenienceFee: cf, totalPrice: total },
      }));
    }
  ...
  const handleProceedToPayment = (bus) => {
    ...
    if (!data || !bus) return toast.error("Booking data not found.");
    ...
    if (totalPrice <= 0 && selectedSeats.length > 0)
      return toast.error("Price could not be determined. Please re-select points.");
    ...
    return (
      <motion.div variants={containerVariants} initial="hidden" animate="visible">
        {visibleBuses.map((bus) => {
          ...
          const busData = busSpecificBookingData[key] || {
            selectedSeats: [],
            seatGenders: {},
            selectedBoardingPoint: bus.boardingPoints?.[0] || null,
            selectedDroppingPoint: bus.droppingPoints?.[0] || null,
          ...
          return (
            ...
            >
              {/* Mobile card (unchanged structure, tuned spacing) */}
              <div
                className={`md:hidden block ${isSoldOut ? "opacity-60 bg-gray-50" : "cursor-pointer"}`}
                onClick={() => {
                  if (!isSoldOut) {
                    navigate(`/book/${bus._id}?date=${searchDateParam}&departureTime=${bus.departureTime}`);
                  }
                }}
              >
                <div className="p-4">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 text-gray-800">
                        <span className="text-[18px] font-medium tabular-nums">{bus.departureTime}</span>
                        <span className="text-sm text-gray-400">—</span>
                        <span className="text-[18px] font-medium tabular-nums">{bus.arrivalTime}</span>
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                  <div className="md:col-span-2">
                    <div className="flex items-center gap-4 mb-3">
                      <div className="flex-shrink-0 w-16 h-16 flex items-center justify-center">
                        {bus.operatorLogo ? (
                          <img src={bus.operatorLogo} alt={`${bus.name} logo`} className="w-full h-full object-contain" />
                        ) : (
                          <FaBus className="text-3xl" style={{ color: neutralMuted }} />
                        )}
                      </div>
                      <div>
                        <h3 className="text-base font-semibold" style={{ color: neutralTitle }}>
                          {bus.name}
                        </h3>
                        ...
                        {bus.liveTracking && (
                          <p className="text-xs font-medium mt-1 flex items-center gap-1" style={{ color: neutralMuted }}>
                    ...
                    <div className="mb-1">
                      <div className="flex items-center">
                        <div className="flex flex-col min-w-[84px]">
                          <span className="text-[11px] uppercase tracking-wide" style={{ color: neutralMuted }}>
                            Departs
                          </span>
                          <span className="text-xl font-semibold tabular-nums" style={{ color: neutralTitle }}>
                            {bus.departureTime}
                          </span>
                        </div>
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-3 text-xs" style={{ color: neutralMuted }}>
                        <span className="inline-flex items-center gap-1">
                  ...
                  <div className="flex flex-col items-start md:items-end">
                    ...
                    <div className="mt-2 inline-block text-right">
                      ...
                      <div className="leading-tight">
                        <span className="text-[11px] font-medium mr-1 align-top" style={{ color: neutralMuted }}>
                          Rs.
                        </span>
                        <span className="text-2xl font-bold tabular-nums" style={{ color: neutralTitle }}>
                          {displayPrice}
                        </span>
                      </div>
                      <div className="text-[11px] font-medium mt-0.5" style={{ color: neutralMuted }}>
                        Onwards
                      </div>
                    </div>
                    <motion.button
                    ...
                    >
                      {isSoldOut ? "Sold Out" : expandedBusId === key ? "Hide Seats" : "View seats"}
                    </motion.button>
                  </div>
                </div>
                        ...
                        <div className="col-span-1 flex flex-col gap-4">
                          <div className="bg-gray-50 p-3 sm:p-4 rounded-lg border border-gray-200 h-full">
                            ...
                            <SeatLayout
                              ...
                              selectedSeatGenders={busData.seatGenders}
                            />
                          </div>
                        </div>
                        <div className="col-span-1 flex flex-col gap-4">
                          <div className="w-full mx-auto xs:max-w-xs sm:max-w-sm">
                            <PointSelection
                              ...
                              selectedBoardingPoint={busData.selectedBoardingPoint}
                              setSelectedBoardingPoint={(p) => handleBoardingPointSelect(bus, p)}
                              selectedDroppingPoint={busData.selectedDroppingPoint}
                          ...
                          <div className="w-full mx-auto xs:max-w-xs sm:max-w-sm">
                            <BookingSummary
                              ...
                              convenienceFee={busData.convenienceFee}
                              totalPrice={busData.totalPrice}
                              onProceed={() => handleProceedToPayment(bus)}
                              boardingPoint={busData.selectedBoardingPoint}
                              droppingPoint={busData.selectedDroppingPoint}
                            />
                          </div>
                        </div>
                      </div>
  ...
  return (
    <div
      className="flex flex-col min-h-screen font-sans min-h-[100svh]"
      style={{ backgroundColor: PALETTE.bgLight, minHeight: "100dvh" }}
    >
      <Toaster position="top-right" />

      {/* iPhone notch tint: fixed overlay so the status area is always white (like redBus) */}
      <div
        className="lg:hidden fixed inset-x-0 top-0 z-[60] pointer-events-none"
        style={{ height: "env(safe-area-inset-top)", backgroundColor: PALETTE.white }}
      />
      {/* spacer so content doesn't sit under the fixed tint */}
      <div className="lg:hidden" style={{ height: "env(safe-area-inset-top)" }} />

      {/* ======= HEADER (mobile refined, desktop unchanged) ======= */}
      <div className="w-full border-b" style={{ backgroundColor: PALETTE.white, borderColor: PALETTE.borderLight }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="block lg:hidden py-2">
            ...
            {/* Route title and count */}
            <div className="mt-2">
              <h1 className="text-[22px] font-bold tracking-tight" style={{ color: PALETTE.textDark }}>
          ...
          <div className="hidden lg:block py-3">
            ...
            {!loading && !fetchError && (
              <p className="text-sm text-gray-500 mb-1">{sortedBuses.length} buses</p>
            )}
          </div>
        </div>
      </div>

      {/* Sticky modify bar (desktop only) */}
      <div
      ...
      {/* Bottom safe area spacer so content never hits the iPhone home bar */}
